...
export default function ItemList({items, setItems, setClickedItem}) {
    ...
    const renderItems = (items, sortBy) => {
        ...
        if (sortBy === "group") {
            ...
            return items.map((item) => (
                <Item key={item.id} item={item} showGroup={true} setClickedItem={setClickedItem}/>
            ));
        }
    };
    ...
    return (
      <div>
        <div className="flex flex-row items-center justify-start w-100 mx-2 px-2">
          <p>Sort By: </p>
          <button
